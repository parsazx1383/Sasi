...
ZARINPAL_SANDBOX = False  # در حالت تست True شود
...
def get_gateway_status(gateway_name="zarinpal"):
    """
    بررسی وضعیت درگاه پرداخت
    """
    result = get_data(f"SELECT * FROM gateway_settings WHERE gateway_name = '{gateway_name}'")
    if result and result["is_active"]:
        return {
            "active": True,
            "merchant_id": result["merchant_id"],
            "sandbox": result["sandbox_mode"]
        }
    return {"active": False}
