...
def generate_payment_invoice(user_id, amount, description=None, email=None, mobile=None):
    """
    تولید لینک پرداخت زرین‌پال - نسخه 4 API
    """
    ...
    # آماده‌سازی داده‌ها برای نسخه 4
    merchant_id = gateway_status.get("merchant_id", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
    
    # اطمینان از اینکه merchant_id معتبر است
    if not merchant_id or merchant_id == "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx":
        return {
            "success": False,
            "message": "مرچنت کد تنظیم نشده است. لطفا در پنل مدیریت مرچنت کد معتبر وارد کنید."
        }
    
    # برای نسخه 4، کال‌بک URL اختیاری نیست - باید حتما ارسال شود
    # اگر کال‌بک ندارید، از ربات تلگرام استفاده کنید
    callback_url = "https://google.com"  # یا هر URL معتبر دیگر
    
    # ساخت data به فرمت نسخه 4
    data = {
        "merchant_id": merchant_id,
        "amount": amount * 10,  # تبدیل به ریال
        "callback_url": callback_url,
        "description": description or "خرید اشتراک دستیار تلگرام",
        "metadata": {
            "user_id": str(user_id),
    ...
    # حذف فیلدهای None از metadata
    if not mobile:
        del data["metadata"]["mobile"]
    if not email:
        del data["metadata"]["email"]
    
    try:
        # تنظیم هدرهای درخواست برای نسخه 4
        headers = {
            "Content-Type": "application/json",
            "Accept": "application/json",
            "User-Agent": "TelegramSelfBot/1.0"
        }
        ...
        # لاگ پاسخ برای دیباگ
        print(f"Zarinpal v4 Response Status: {response.status_code}")
        print(f"Zarinpal v4 Response Content: {response.text}")
        
        # بررسی وضعیت HTTP
        if response.status_code != 200:
            return {
                "success": False,
                "message": f"خطا در ارتباط با زرین‌پال. کد وضعیت: {response.status_code}"
            }
        ...
        try:
            ...
            return {
                "success": False,
                "message": f"پاسخ نامعتبر از زرین‌پال: {str(e)}"
            }
        
        # بررسی ساختار پاسخ نسخه 4
        if result.get("data") and result["data"].get("code") == 100:
            ...
            # کدهای خطای نسخه 4
            error_code = result.get("data", {}).get("code", result.get("errors", {}).get("code"))
            ...
            # اضافه کردن جزئیات خطا اگر موجود باشد
            if result.get("errors"):
                error_details = result["errors"].get("message", "بدون جزئیات")
                error_msg += f" - {error_details}"
            
            return {
                "success": False,
                "message": f"خطا در ایجاد لینک پرداخت: {error_msg}"
            }
            
    except requests.exceptions.Timeout:
        return {
            "success": False,
            "message": "زمان اتصال به زرین‌پال به پایان رسید"
        }
    except requests.exceptions.ConnectionError:
        return {
            "success": False,
            "message": "خطا در اتصال به زرین‌پال. لطفا اینترنت خود را بررسی کنید"
        }
    except Exception as e:
        return {
            "success": False,
            "message": f"خطا در ارتباط با زرین‌پال: {str(e)}"
        }

def verify_payment(authority, amount):
    """
    تأیید پرداخت زرین‌پال - نسخه 4 API
    """
    # دریافت تنظیمات درگاه
    gateway_status = get_gateway_status()
    ...
    try:
        ...
        print(f"Zarinpal Verify Response: {response.text}")
        
        if response.status_code != 200:
            return {
                "success": False,
                "message": f"خطا در تأیید پرداخت. کد وضعیت: {response.status_code}"
            }
        ...
        if result.get("data") and result["data"].get("code") in [100, 101]:
            ...
            return {
                "success": True,
                "ref_id": ref_id,
                "message": "پرداخت با موفقیت تأیید شد" if result["data"]["code"] == 100 else "پرداخت قبلاً تأیید شده است"
            }
        else:
            error_code = result.get("data", {}).get("code", result.get("errors", {}).get("code"))
...
async def callback_handler(c, call):
    ...
    if data == "BuySub" or data == "Back2":
        ...
        # 🔍 بررسی هوشمند وضعیت درگاه
            ...
            if gateway_status["active"]:
            # پیام برای پرداخت آنلاین
                status_msg = "🟢 **درگاه پرداخت آنلاین فعال است**\n\n"
                status_msg += "انتقال به درگاه امن زرین‌پال..."
            
            # منوی اشتراک‌ها برای پرداخت آنلاین
                await app.edit_message_text(chat_id, m_id,
                    f"{status_msg}\n\n"
                    "**💎 لطفا مدت اشتراک مورد نظر را انتخاب کنید:**",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton(text=f"📅 1 ماه - {prices['1month']} تومان", callback_data=f"PayGateway-30-{prices['1month']}-{card_id}")],
                        [InlineKeyboardButton(text=f"📅 2 ماه - {prices['2month']} تومان", callback_data=f"PayGateway-60-{prices['2month']}-{card_id}")],
                        [InlineKeyboardButton(text=f"📅 3 ماه - {prices['3month']} تومان", callback_data=f"PayGateway-90-{prices['3month']}-{card_id}")],
                        [InlineKeyboardButton(text=f"📅 4 ماه - {prices['4month']} تومان", callback_data=f"PayGateway-120-{prices['4month']}-{card_id}")],
                        [InlineKeyboardButton(text=f"📅 5 ماه - {prices['5month']} تومان", callback_data=f"PayGateway-150-{prices['5month']}-{card_id}")],
                        [InlineKeyboardButton(text=f"📅 6 ماه - {prices['6month']} تومان", callback_data=f"PayGateway-180-{prices['6month']}-{card_id}")],
                        [InlineKeyboardButton(text="(🔙) بازگشت", callback_data="BuySub")]
                    ]))
            else:
            # پیام برای پرداخت دستی
                status_msg = "🔴 **درگاه پرداخت غیرفعال است**\n\n"
                status_msg += "از روش پرداخت کارت به کارت استفاده کنید..."
            
            # منوی اشتراک‌ها برای پرداخت دستی
                await app.edit_message_text(chat_id, m_id,
                    f"{status_msg}\n\n"
                    "**💎 لطفا مدت اشتراک مورد نظر را انتخاب کنید:**",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton(text=f"📅 1 ماه - {prices['1month']} تومان", callback_data=f"Sub-30-{prices['1month']}")],
                        [InlineKeyboardButton(text=f"📅 2 ماه - {prices['2month']} تومان", callback_data=f"Sub-60-{prices['2month']}")],
                        [InlineKeyboardButton(text=f"📅 3 ماه - {prices['3month']} تومان", callback_data=f"Sub-90-{prices['3month']}")],
                        [InlineKeyboardButton(text=f"📅 4 ماه - {prices['4month']} تومان", callback_data=f"Sub-120-{prices['4month']}")],
                        [InlineKeyboardButton(text=f"📅 5 ماه - {prices['5month']} تومان", callback_data=f"Sub-150-{prices['5month']}")],
                        [InlineKeyboardButton(text=f"📅 6 ماه - {prices['6month']} تومان", callback_data=f"Sub-180-{prices['6month']}")],
                        [InlineKeyboardButton(text="(🔙) بازگشت", callback_data="BuySub")]
                    ]))
    ...
    # ایجاد لینک پرداخت
        ...
        # نمایش فاکتور به کاربر
            invoice_message = f"""
**📋 فاکتور پرداخت**

• **مبلغ:** `{cost:,}` تومان
• **مدت اشتراک:** `{expir_count}` روز
• **بانک:** `{bank_name}`
• **شماره کارت:** `{masked_card}`
• **شناسه پرداخت:** `{payment_result['authority']}`

برای پرداخت روی دکمه زیر کلیک کنید:
"""
        
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("💳 پرداخت آنلاین", url=payment_result["payment_url"])],
                [InlineKeyboardButton("✅ اعتبارسنجی پرداخت", callback_data=f"VerifyPayment-{payment_result['authority']}")],
                [InlineKeyboardButton("❌ انصراف", callback_data=f"CancelPayment-{payment_result['authority']}")]
            ])
    ...
    # بررسی وضعیت تراکنش
        if transaction["status"] == "success":
            await app.answer_callback_query(call.id, text="این تراکنش قبلاً تأیید شده است", show_alert=True)
            return
    
        await app.answer_callback_query(call.id, text="در حال تأیید پرداخت...", show_alert=False)
...
• **انقضای سلف شما {transaction['plan_days']} روز اضافه گردید**
...
"""
            ...
            await app.edit_message_text(chat_id, m_id, success_message)
        ...
        # پرداخت ناموفق
            ...
            await app.answer_callback_query(call.id, text=f"پرداخت تأیید نشد: {verify_result['message']}", show_alert=True)

    elif data.startswith("CancelPayment-"):
    # لغو پرداخت
        authority = data.split("-")[1]
    
        update_payment_transaction(authority, "cancelled")
    
        keyboard = get_main_keyboard(chat_id)
        await app.edit_message_text(chat_id, m_id, "**پرداخت لغو شد.**", reply_markup=keyboard)
...
""", reply_to_message_id=mess.id, reply_markup=InlineKeyboardMarkup(
    ...
    elif user["step"] == "edit_merchant_id":
